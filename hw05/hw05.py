...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    counts = []
    def withdraw(amount, passwords):
        nonlocal balance
        nonlocal counts
        if len(counts) == 3:
            return f"Too many incorrect attempts. Attempts: {counts}"
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def inner(remainer, m):
        i = 1
        while True:
            if remainer == 0:
                yield m * i + remainer
            else:
                yield m * (i-1) + remainer
            i += 1

    for remainder in range(m):
        yield inner(remainder, m)
